...
\begin{abstract}
% Zayn is happy with the abstract
...
% This year we focused on individualizing our mechanical, electrical, and software solutions, increasing our domain knowledge over the components we use and reducing dependency on COTS (commercial-off-the-shelf parts).
%-- ROS 2 Migration
%-- Custom Thruster Controller board (ESP32) as well as custom Thruster Controller code
%-- New refreshed onboarding
%-- New teleop code.
%-- DVL?????
%-- fuses and electrical safety stuff

\end{abstract}

\section{Competition Strategy} \label{sec:comp_strat}
This year marks our last year focusing our development and competition strategy around our 2023 hull. As such, we catered to maximizing the competitive potential of our current submarine by improving AUV capabilities and system robustness. Some areas that we identified as established strengths include our machine learning pipeline, which we were able to quickly train and utilize at previous competitions, and our task planner that maps out the failure cases and fallback states for each task. To make our system electrically robust and better organize our Electronic Speed Controllers (ESCs), we developed custom motor controller PCBs. To refine AUV capabilities, we developed a custom PID controller, custom motor controller code, and custom teleoperation code to enhance flexibility and reduce unnecessary reliance on external packages. We also migrated our software stack to ROS 2 \cite{b6} after ROS 1 reached end-of-life in June 2025.
...
Last year, we had limited success in reaching the bins due to difficulty navigating through the slalom poles. Provided we can successfully complete slalom navigation using machine-learning, this year, we intend to complete the bins task for the first time as a stretch goal. We intend to maximize points by dropping two markers into the sides of the bin corresponding to our chosen side of the gate. For the past two years, we have experimented with various designs. Our most recent iteration will is able to drop each of the markers one at a time. Once we reach the bins, we will employ machine learning-based object detection to find the corresponding "Survey \& Repair" or "Search \& Rescue" image. Next, to center on the bin, we alternate descending and re-centering on the bin, switching between the two based on the visual distance to the center of the bin in the bottom camera feed. We do these movements separately rather than simultaneously to prevent the camera from potentially losing sight of the bin.
...
\begin{figure}[h]
    \centerline{\includegraphics[scale=0.22]{images/CarterGrabberV4.png}}
...
\label{sssec:dropper}
This year, we also implemented a marker dropper subsystem (Fig \ref{fig:dropper}). This year, we have chosen to utilize a single dropper that contains two markers. We have designed a mechanism that allows the markers to only be dropped one at a time, and cannot be unintentionally dropped. The dropper is 3-D printed via FDM out of PLA. To hold our markers in place, we have another PLA part that functions as a rotating, T-shaped guide that connects directly to our servo and holds the markers away from the opening on the bottom of our base. 3-D printing was used for the design because it is easy to produce, modify, and replace, and our part does not undergo a lot of stress.

\begin{figure}[h]
    \centerline{\includegraphics[scale=0.22]{images/Marcus_Dropper.png}}
...
Unlike the Montemayor, our new sub does not store the battery in the main hull. Instead, we have a separate acrylic enclosure for it, recovered from our previous AUVs. Doing this will allow us to keep our center of gravity along the midline of the sub, and also keep it low for hydrostatic stability. %Bunch more to talk about, eg buoyancy/ballasting,thrusters, degrees of freedom, frame mounting, etc
%Replace figure below with image of new hull CAD if possible (File-->Export-->Image in NX), then import the file into the images file in this doc and change file name below to match image
% \begin{figure}[h]
...
% Listing what we need for electrical:
% More on voltage monitor contraption pls
% \subsubsection{Newly Created Sub-Team} % I took out this section
...
% Zayn (05/09/2026): We need to decide what to keep/remove below v v v
...
% Zayn/Colten (05/09/26):
% GPU used is Jetson Orin Nano
% Summarize info about the PixHawk
% Serial communication happens with the pysserial package (not ROS specific)
% ESP 32 doesn't care about the ROS (written using Arduino code)
% Keep the section on the Flight Controller (maybe summarize)
% Overhaul the color detection in favor of object detection (more in Muskaan's wheel house)
%
\label{ssec:software}
...
\begin{figure}[htbp]
    \centerline{\includegraphics[height=6.5cm, keepaspectratio=false]{images/software_architecture.png}}
    \caption{High-level Software Architecture}
    \label{fig:software_architecture}
\end{figure}

\subsubsection{Computing Architecture}
% Zayn (05/09/26): Needs a LOT of work
% Colten's taking this
% Colten (05/09/26): Currently the outdated parts are stripped out, will expand more later
\label{sssec:comp_arch}
Our computing architecture features a distributed layout to enable greater flexibility and faster iteration. Most of our computing is performed on our Jetson Orin Nano. This is the most powerful computer in the system, so it runs our computer vision, machine learning, high-level planning, localization, and control modules. Expanding on the control modules, our motor control system utilizes custom solutions: an Inertial Measurement Unit (IMU), a thruster control board and driver, and thruster mixing code. All of this now also runs on the Jetson Orin Nano. For interfacing with our low-power devices---namely, our depth sensor, hall-effect sensors, and indicator lights-–-we use an Arduino, which the Jetson communicates with via \verb|pyserial|.

\subsubsection{Flight Controller}
% Zayn (05/09/26): I like this section. I think maybe we can summarize it a little but I wouldn't mind keeping this section as-is.
% Colten's taking this
% Colten (05/09/26): Cut out mentions of the Pixhawk Flight Controller, besides that, only changed some verbage
...
\begin{figure}[htbp]
    \centerline{\includegraphics[scale=0.28]{images/motor_fits.png}}
...
Thus, this year, we have switched to a machine learning approach to detect the path marker. We fine-tune a YOLOv8 segmentation model \cite{b7} on images taken at competition. This model both detects our desired object and gives us a mask for the detection. We then use geometrical methods from OpenCV \cite{b8} to determine the orientation of the path marker from the mask, thus informing our AUV's trajectory.
...
% Zayn (05/09/2026): It might be useful to talk more about generation of test sets for ML depending on how much space we have left.
% This was from 2 years ago and can be streamlined a bit
% Muskaan (05/13/2026) I have tried to streamline the following paragraph:
...
% this section has been updated by muskaan may 2026
...
The task planner is the authority for coordinating system components to coordinate high-level logic, like navigating competition tasks. Our task planner is a state machine, where the AUV is assigned a current state that dictates its logic and may produce a state transition if certain conditions are met.
...
In the past few years, we have been significantly improving our state machine framework to reduce operational friction and catch errors before water-test time. The task planner supports overriding parameters on a per transition map basis, allowing us to set different target depths, angles, distances etc. between testing and competition environments, significantly reducing testing preparation overhead. We also added the ability to start running the state machine from any state, rather than just the predefined start state, which helps during tests to isolate and test potentially faulty units of behavior. Finally, all of our planning code can be checked with a static python type checker, \verb|mypy|. This notifies us ahead of time of potentially faulty code, making our overall codebase more reliable. The additional type safety also helps our development environment to provide much more relevant auto-complete suggestions, increasing developer productivity. 

\section{Testing Strategy}
\label{sec:testing}

\subsection{Teleoperation Framework}
% teleop updated may 2026 by muskaan :)
\label{ssec:teleop}
...
% Zayn (05/09/2026): I want to add acknoledgement for the Ford Robotics Building Pool staff, they have been super helpful
% (05/20/2026): Muskaan has added FRB staff names
...
\begin{tabularx}{\textwidth}{@{} l l Y Y r c @{}}
    ...
    %   -------------------------------------------------------------------------------------------------------------------------------------------
        ...
        \midrule
        ASV Hull Form           & American Tooling              & Custom              & 6061 aluminum    & \$3,250.00 & 2022             \\ 
      ...
      & \textemdash  & 2023          \\ 
        ...
        Testing Controller & Logitech & Logitech F310 & 10 programmable buttons, 4 switch floating D-Pad & \$19.99 & 2021-2026 \\
